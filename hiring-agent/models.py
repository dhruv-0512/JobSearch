...
class DeepSeekProvider:
    ...
    def chat(
        ...
        **kwargs
        ...
        """Send a chat request to DeepSeek via OpenAI-compatible API."""
        extra_params = {}
        if options:
            if "temperature" in options:
                extra_params["temperature"] = options["temperature"]
            if "top_p" in options:
                extra_params["top_p"] = options["top_p"]

        # DeepSeek supports response_format for structured JSON output
        response_format = None
        if "format" in kwargs and isinstance(kwargs["format"], dict):
            # JSON schema format — use json_object mode for compatibility
