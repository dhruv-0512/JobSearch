...
class PDFHandler:
    ...
    def extract_json_from_pdf(self, pdf_path: str) -> Optional[JSONResume]:
        try:
            ...
            json_resume = self._extract_all_sections_separately(text_content)

            # Direct link extraction fallback to catch masked hyperlinks
            if json_resume:
                profiles = []
                if json_resume.basics:
                    profiles = json_resume.basics.profiles or []
                        ...
                        for page in doc:
                            for link in page.get_links():
                                ...
                                if uri and "github.com/" in uri.lower():
                                    direct_github_links.append(uri)
                        doc.close()
                    except Exception as e:
                        logger.error(f"Direct link extraction error: {e}")

                    if direct_github_links:
                        first_link = direct_github_links[0].strip()
                        ...
                        logger.info(f"✨ Direct Link Injection: Found GitHub link {first_link} and username {username}")

                        from models import Profile, Basics
                        if not json_resume.basics:
                            json_resume.basics = Basics(
                                name=os.path.basename(pdf_path).replace(".pdf", ""),
                                profiles=[]
                            )

                        if json_resume.basics.profiles is None:
                            json_resume.basics.profiles = []

                        json_resume.basics.profiles.append(Profile(
                            network="Github",
                            username=username,
                            url=first_link
                        ))
