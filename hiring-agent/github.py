...
def _fetch_github_api(api_url, params=None):
    ...
    if github_token:
        ...
        if github_token and "your_github_token" not in github_token and github_token != "ghp_xxxxxxxxxxxx":
    ...
    # Check if the token is unauthorized (401) and retry without it
    if response.status_code == 401 and "Authorization" in headers:
        logger.warning("⚠️  GitHub API returned 401 (Unauthorized) with token. Retrying WITHOUT token...")
